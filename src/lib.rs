...
impl TokenTreeIter {
    fn new(input: Literal) -> Self {
        let mut hex_symbol_free = input.to_string().replace("#", "").to_lowercase();
        let count = hex_symbol_free.chars().count();

        if count == 5 {
            let temp_string = hex_symbol_free.replace('"', "");
            hex_symbol_free = temp_string.chars()
                .map(|c| c.to_string()
                .repeat(2))
                .collect::<Vec<String>>()
                .join("");

            hex_symbol_free.insert(0, '"');
            hex_symbol_free.push('"');
        } else if count != 7 {
            panic!("string length should equal to 3 or 6");
        }
        
        let mut buf: Vec<u8> = hex_symbol_free.into();
...
impl Iterator for TokenTreeIter {
    ...
    fn next(&mut self) -> Option<TokenTree> {
        let v = if self.is_punct {
            ...
            let val = (p1 << 4) + p2;
            TokenTree::Literal(Literal::f32_suffixed(val as f32 / 255.))
        };
